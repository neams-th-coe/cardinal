...
NekInitAction::act()
{
  if (_type == "NekRSProblem" || _type == "NekRSStandaloneProblem" || _type == "NekRSSeparateDomainProblem")
  {
    ...
    if (!casename_on_command_line && !_casename_in_input_file)
      mooseError("All inputs using 'NekRSProblem' or 'NekRSStandaloneProblem' must pass '--nekrs-setup <case>' on "
  ...
  if (_type == "NekRSProblem" || _type == "NekRSSeparateDomainProblem")
  {
    ...
    if (!scratch_available)
      mooseError("The nrs_t.usrwrk and nrs_t.o_usrwrk arrays are automatically allocated by Cardinal!\n"
        "The first 'slice' (i.e. first nrs->cds->fieldOffset[0] entries) is reserved for heat flux from MOOSE\n"
        "(if not using boundary coupling, this entry is not touched), while the second 'slice' (i.e. the second\n"
        "nrs->cds->fieldOffset[0] entries) is reserved for a volumetric heat source from MOOSE (if not using\n"
        "volume coupling, this entry is not touched).\n\n"
        "At present, these scratch space arrays are allocated to have size 7 * nrs->cds->fieldOffset[0];\n"
        "if you wish to use this scratch space for other purposes, such as storing a wall distance, please\n"
        "remove the manual allocation of the space in your user files, and be sure to only write beginning at\n"
        "2 * nrs->cds->fieldOffset[0] such that the space reserved for coupling data is untouched.");
