    ...
    if (t->hasScore(score))
    {
      ...
      for (unsigned int ext_bin = 0; ext_bin < vars.size(); ++ext_bin)
      {
        ...
        score_vars.emplace_back(
            dynamic_cast<const MooseVariableFE<Real> *>(&getVariable(tid, vars[ext_bin])));
      }
    }
  }
    ...
    if (t->hasScore(score))
    {
      ...
      for (unsigned int ext_bin = 0; ext_bin < vars.size(); ++ext_bin)
      {
        if (skip_func_exp && t->extBinSkipped(ext_bin))
          continue;
        score_vars.emplace_back(
            &(dynamic_cast<MooseVariableFE<Real> *>(&getVariable(tid, vars[ext_bin]))
                  ->slnNeighbor()));
      }
    }
  }
...
OpenMCCellAverageProblem::tallyMultiplier(unsigned int global_score) const
{
  if (!isHeatingScore(_all_tally_scores[global_score]))
  {
    ...
    // - Reaction rate scores & 'inverse-velocity' have units of reactions/src (OpenMC) or reactions/s (Cardinal).
    // - 'damage-energy' has units of eV/src (OpenMC) or eV/s (Cardinal). While the units of damage-energy
    //   are the same as a heating tally, we don't normalize it like one as it's used as an intermediate to compute DPA.
