...
OpenMCProblem::validParams()
{
  ...
  params.addParam<std::vector<Point>>("centers",
                                      "Coordinates of cell centers to transfer data with");
  params.addParam<std::vector<FileName>>("centers_file",
                                         "Alternative way to provide the coordinates of cells "
  ...
  params.addParam<bool>(
      "check_tally_sum", true, "Check heating tally consistency between timesteps");
  return params;
}
    ...
    _check_tally_sum(getParam<bool>("check_tally_sum"))
{
  ...
  if (_tallyType == tally::cell)
  {
    if (!isParamValid("volumes"))
      paramError("volumes",
                 "Wth cell-based tallies, the volumes of each MOOSE receiving mesh "
                 "must be provided!");
    ...
    if ((_volumes.size() > 1) && (_volumes.size() != _centers.size()))
      paramError("volumes",
                 "When more than one volume is provided, the length must match "
  ...
  if (_tallyType == tally::mesh)
  {
    ...
    if (_meshTemplateFilename.empty())
      paramError("mesh_template",
                 "When using a mesh tally, the 'mesh_template' parameter cannot be empty!");
  }
...
OpenMCProblem::~OpenMCProblem() { openmc_finalize(); }
  ...
  else if (isParamValid("centers_file"))
  {
    ...
    for (const auto & f : centers_file)
    {
      ...
      if (file.numEntries() % DIMENSION != 0)
        paramError("centers_file",
                   "Number of entries in 'centers_file' ",
                   f,
                   " must be divisible by 3 to give x, y, and z coordinates");
...
double
OpenMCProblem::kappa_fission_total() const
{
...
void
OpenMCProblem::addExternalVariables()
{
...
OpenMCProblem::initialSetup()
{
  ...
  _console << "Initializing OpenMC problem with " << Moose::stringify(n_centers)
           << " coupling points..." << std::endl;
}

void
OpenMCProblem::syncSolutions(ExternalProblem::Direction direction)
{
  switch (direction)
  {
    case ExternalProblem::Direction::TO_EXTERNAL_APP:
    {
      ...
      */
      ...
      _console << "Temperature min/max values from MOOSE: " << minimum << ", " << maximum
               << std::endl;
...
void
OpenMCProblem::checkTallySum(double tally_sum) const
{
  ...
  if (std::abs(kf_total - tally_sum) / kf_total > openmc::FP_REL_PRECISION)
  {
    mooseError("Heating tally kappa-fission does not match the global kappa-fission value.\n"
               "Global value: " +
               Moose::stringify(kf_total) + "\nTally sum: " + Moose::stringify(tally_sum));
  }
}

std::vector<double>
OpenMCProblem::meshHeatSource()
{
  // determine the size of the heat source
  size_t heat_source_size = 0;
  for (const auto & t : _tallies)
  {
    heat_source_size += t->n_filter_bins();
  }
...
double
OpenMCProblem::getCellVolume(int cellIndex)
{
