...
OpenMCCellTranslator::validParams()
{
  ...
  params.addRequiredParam<std::vector<int32_t>>(
      "cell_ids", "List of OpenMC cell IDs whose filled universes will be translated.");

  params.addRequiredParam<PostprocessorName>("x_translation",
                                             "Postprocessor giving x-translation in mesh units.");
  params.addRequiredParam<PostprocessorName>("y_translation",
                                             "Postprocessor giving y-translation in mesh units.");
  params.addRequiredParam<PostprocessorName>("z_translation",
...
OpenMCCellTranslator::execute()
{
  ...
  _console << "Setting OpenMC cell translations for " << _cell_ids.size() << " cell(s) to (" << dx
           << ", " << dy << ", " << dz << ") in OpenMC geometry units." << std::endl;
