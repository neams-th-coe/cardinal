...
fuel_to_coolant_distance = (
    0.0188  # distance between center of fuel compact and coolant channel (m)
)
...
def unit_cell(n_ax_zones, n_inactive, n_active):
    ...
    fuel_cyl2 = openmc.ZCylinder(r=0.5 * fuel_channel_diam)
    ...
    homogeneous_fuel_univ = openmc.Universe(cells=[openmc.Cell(fill=m_fuel)])
    different_homogeneous_fuel_univ = openmc.Universe(cells=[openmc.Cell(fill=m_fuel)])
    ...
    for z_min, z_max in zip(axial_coords[0:-1], axial_coords[1:]):
        ...
        fuel_ch_cell = openmc.Cell(
            region=-fuel_cyl, fill=different_homogeneous_fuel_univ
        )
        uniform_fuel_ch_cell = openmc.Cell(
            region=-fuel_cyl2, fill=homogeneous_fuel_univ
        )

        fuel_ch_matrix_cell = openmc.Cell(region=+fuel_cyl, fill=m_water_matrix)
        fuel_ch_matrix_cell2 = openmc.Cell(region=+fuel_cyl2, fill=m_water_matrix)

        # create two different "fuel" universes -
        # This will trigger an error in the contained cells mapping, since there are different
        # cells and instances in these two different universes
        fuel_u = openmc.Universe(cells=[fuel_ch_cell, fuel_ch_matrix_cell])
        fuel_u2 = openmc.Universe(cells=[uniform_fuel_ch_cell, fuel_ch_matrix_cell2])

        coolant_u = openmc.Universe(cells=[c_cell, graphite_cell])
        lattice_univs.append([[fuel_u] * 5 + [fuel_u2], [coolant_u]])
...
def main():
    ...
    ap.add_argument(
        "-a", dest="n_active", type=int, default=20, help="Number of active cycles"
    )
