...
def unit_cell(n_ax_zones, n_inactive, n_active):
    ...
    c_triso_matrix = openmc.Cell(
        name="c_triso_matrix", fill=m_water_matrix, region=+s_fuel
    )
    ...
    rand_spheres = openmc.model.pack_spheres(
        radius=radius_pyc_outer, region=r_triso, pf=triso_pf
    )
    random_trisos = [
    ...
    triso_lattice = openmc.model.create_triso_lattice(
        random_trisos, llc, pitch, triso_lattice_shape, m_water_matrix
    )
