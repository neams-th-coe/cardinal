...
def water_density(T):
    ...
    """
    rho = 0.001 * (
        0.14395 / math.pow(0.0112, 1.0 + math.pow(1.0 - T / 649.727, 0.05107))
    )
    return rho
...
water_surface = openmc.ZCylinder(
    r=pitch / 2.0, name="Water surface", boundary_type="white"
)
fuel_cell = openmc.Cell(fill=uo2, region=-pellet_surface, name="Fuel")
clad_cell = openmc.Cell(
    fill=zircaloy, region=+pellet_surface & -pincell_surface, name="Clad"
)
water_cell = openmc.Cell(
    fill=water_materials, region=+pincell_surface & -water_surface, name="Water"
)
repeatable_univ = openmc.Universe(cells=[fuel_cell, clad_cell, water_cell])

outer_cell = openmc.Cell(
    fill=water_materials[0], region=+pincell_surface & -water_surface, name="Outside"
)
