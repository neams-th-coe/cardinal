...
water_surface = openmc.rectangular_prism(
    width=pitch, height=pitch, boundary_type="white"
)
lattice_cell = openmc.Cell(
    fill=lattice, region=-surface_to_enclose_lattice & +bottom & -top
)
water_cell = openmc.Cell(
    fill=water,
    region=+surface_to_enclose_lattice & water_surface & +bottom & -top,
    name="Water",
)
