...
# Generate the multigroup cross section data set. The temperature dependence is implemented with
# density feedback, allowing for the use of isothermal MGXS.
groups = openmc.mgxs.EnergyGroups(np.logspace(1, 20, 2))
abs_xsdata = openmc.XSdata('abs', groups, temperatures=[T0])
abs_xsdata.order = 0
...
abs_xsdata.set_total([N0*sigma0], temperature=T0)
abs_xsdata.set_absorption([N0*sigma0], temperature=T0)
