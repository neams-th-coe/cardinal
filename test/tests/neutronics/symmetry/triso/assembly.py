...
def assembly(n_ax_zones, n_inactive, n_active):
    ...
    c_triso_fuel = openmc.Cell(
        name="c_triso_fuel", fill=compact_mats.m_fuel, region=-s_fuel
    )
    c_triso_c_buffer = openmc.Cell(
        name="c_triso_c_buffer",
        fill=compact_mats.m_graphite_c_buffer,
        region=+s_fuel & -s_c_buffer,
    )
    c_triso_pyc_inner = openmc.Cell(
        name="c_triso_pyc_inner",
        fill=compact_mats.m_graphite_pyc,
        region=+s_c_buffer & -s_pyc_inner,
    )
    c_triso_sic = openmc.Cell(
        name="c_triso_sic", fill=compact_mats.m_sic, region=+s_pyc_inner & -s_sic
    )
    c_triso_pyc_outer = openmc.Cell(
        name="c_triso_pyc_outer",
        fill=compact_mats.m_graphite_pyc,
        region=+s_sic & -s_pyc_outer,
    )
    c_triso_matrix = openmc.Cell(
        name="c_triso_matrix", fill=compact_mats.m_graphite_matrix, region=+s_pyc_outer
    )
    ...
    rand_spheres = openmc.model.pack_spheres(
        radius=radius_pyc_outer, region=r_triso, pf=0.05, seed=1
    )
    random_trisos = [
    ...
    triso_lattice = openmc.model.create_triso_lattice(
        random_trisos, llc, pitch, triso_lattice_shape, compact_mats.m_graphite_matrix
    )
    ...
    for z_min, z_max in zip(axial_coords[0:-1], axial_coords[1:]):
        ...
        fuel_ch_matrix_cell = openmc.Cell(
            region=+fuel_cyl, fill=compact_mats.m_graphite_matrix
        )

        poison_matrix_cell = openmc.Cell(
            region=+poison_cyl, fill=compact_mats.m_graphite_matrix
        )

        graphite_cell = openmc.Cell(fill=compact_mats.m_graphite_matrix)

        coolant_matrix_cell = openmc.Cell(
            region=+coolant_cyl, fill=compact_mats.m_graphite_matrix
        )
    ...
    hex_prism = openmc.model.HexagonalPrism(
        bundle_pitch / math.sqrt(3.0), "x", boundary_type="reflective"
    )
    outer_cell = openmc.Cell(
        region=-hex_prism & +min_z & -max_z & +symmetry_plane, fill=hex_lattice
    )
