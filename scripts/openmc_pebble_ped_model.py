...
def report_pebble_cell_level(geom: openmc.Geometry, pebble_cells):
    ...
    if level is not None:
        ...
        raise RuntimeError("Pebble cells were not" "found in the geometry.")
...
program_epilog = (
    "This script expects the presence of a "
    "'pebble_centers_rescaled.txt' file in which "
    "all pebble centers are represented for pebbles "
    "with a radius of 1.5 cm."
)

ap = ArgumentParser(description=program_description, epilog=program_epilog)

ap.add_argument(
    "-e",
    dest="extra_flibe_layer",
    type=float,
    default=0.00001,
    help="Flibe layer thickness",
)
ap.add_argument(
    "-v",
    dest="verbose",
...
ap.add_argument(
    ...
    action="store_true",
    default=False,
    help="Enable random TRISO distribution",
)
ap.add_argument(
    "-b", dest="bc", type=str, default="reflective", help="Reactor top/bottom B.C."
)
ap.add_argument(
    "-p", dest="particles", type=int, default=1000, help="Particles per batch"
)
ap.add_argument("-i", dest="inactive", type=int, default=10, help="Inactive batches")
ap.add_argument("-a", dest="active", type=int, default=40, help="Active batches")
ap.add_argument(
    "-l",
    dest="pshape",
    type=int,
...
ap.add_argument(
    "-f",
    dest="radial_reflector_thickness",
    type=float,
    default=0.0,
    help="Include surrounding reflector on outer periphery",
)
ap.add_argument(
    "-g",
    dest="axial_reflector_thickness",
    type=float,
    default=0.0,
    help="Include surrounding reflector on top/bottom of the bed",
)
args = ap.parse_args()
...
vessel_outer_radius = (
    np.max(np.linalg.norm(pebble_centers[:, :-1], axis=1)) + radius_pebble_outer
)
vessel_inner_radius = (
    np.min(np.linalg.norm(pebble_centers[:, :-1], axis=1)) - radius_pebble_outer
)
...
if verbose:
    ...
    if not random_distribution:
        print(
            "TRISO particles regular lattice pitch_triso_lattice [cm] = {}".format(
                pitch_triso_lattice
            )
        )
    ...
    if radial_reflector_is_present or axial_reflector_is_present:
        ...
        print(
            "Reflector height             [cm] = {}".format(
                reflector_z_max - reflector_z_min
            )
        )
...
m_fuel.add_nuclide(
    "U234", 89.58 * enrichment_uranium * fraction_u234, percent_type="wo"
)
m_fuel.add_nuclide(
    "U235", 89.58 * enrichment_uranium * (1 - fraction_u234), percent_type="wo"
)
...
mols_graphite = (
    (1.0 - reflector_porosity) * (rho_graphite * 1000.0) / graphite_molar_mass
)
...
c_triso_c_buffer = openmc.Cell(
    name="TRISO Graphite Buffer", fill=m_graphite_c_buffer, region=+s_fuel & -s_c_buffer
)
c_triso_pyc_inner = openmc.Cell(
    name="TRISO Pyrolitic Graphite Inner",
    fill=m_graphite_pyc,
    region=+s_c_buffer & -s_pyc_inner,
)
c_triso_sic = openmc.Cell(
    name="TIRSO Silicone Carbide", fill=m_sic, region=+s_pyc_inner & -s_sic
)
c_triso_pyc_outer = openmc.Cell(
    name="TRISO Pyrolitic Graphite Outer",
    fill=m_graphite_pyc,
    region=+s_sic & -s_pyc_outer,
)
c_triso_matrix = openmc.Cell(
    name="TRISO Graphite Matrix", fill=m_graphite_matrix, region=+s_pyc_outer
)
...
c_pebble_inner = openmc.Cell(
    name="Pebble graphite inner region", fill=m_graphite_inner, region=-s_pebble_inner
)
c_pebble_central = openmc.Cell(
    name="Pebble central region (TRISOs)", region=+s_pebble_inner & -s_pebble_central
)
c_pebble_outer = openmc.Cell(
    name="Pebble graphite outer region",
    fill=m_graphite_outer,
    region=+s_pebble_central & -s_pebble_outer,
)
c_pebble_flibe = openmc.Cell(
    name="Pebble exterior (FLiBe)", fill=m_flibe, region=+s_pebble_outer
)
...
if not random_distribution:
    ...
    c_triso_pyc_outer_random = openmc.Cell(
        name="c_triso_pyc_outer_random", fill=m_graphite_pyc, region=+s_sic
    )
    triso_rand_cells = [
        c_triso_fuel,
        c_triso_c_buffer,
    ...
    if not os.path.exists("triso_centers.npy"):
        ...
        spheres_random = openmc.model.pack_spheres(
            radius=radius_pyc_outer,
            region=r_triso_random,
            pf=packing_fraction,
            initial_pf=0.15,
        )
        triso_random = [
            openmc.model.TRISO(radius_pyc_outer, u_triso_random, i)
            for i in spheres_random
        ]
    ...
    if verbose:
        packing_fraction = (
            len(triso_random)
            * radius_pyc_outer**3
            / (radius_pebble_central**3 - radius_pebble_inner**3)
        )
        print(
            "Calculated packing fraction of the TRISO "
    ...
    l_triso = openmc.model.create_triso_lattice(
        triso_random, lower_left, pitch, shape, m_graphite_matrix
    )
...
pebble_univ_cells = [c_pebble_inner, c_pebble_central, c_pebble_outer, c_pebble_flibe]
...
vessel_outer = openmc.ZCylinder(
    x0=vessel_x, y0=vessel_y, r=vessel_outer_radius, boundary_type=vessel_radial_bc
)

vessel_bottom = openmc.ZPlane(z0=vessel_z_min, boundary_type=vessel_axial_bc)
vessel_top = openmc.ZPlane(z0=vessel_z_max, boundary_type=vessel_axial_bc)
...
if radial_reflector_is_present and (not axial_reflector_is_present):
    vessel_inner = openmc.ZCylinder(
        x0=vessel_x, y0=vessel_y, r=vessel_inner_radius, boundary_type="transmission"
    )
    ...
    reflector_outer = openmc.ZCylinder(
        x0=vessel_x, y0=vessel_y, r=reflector_outer_radius, boundary_type="vacuum"
    )
    outer_reflector_region = -reflector_outer & axial_plane & +vessel_outer
    inner_reflector_region = -vessel_inner & axial_plane
    outer_reflector_cell = openmc.Cell(
        name="Outer Reflector", region=outer_reflector_region, fill=m_reflector
    )
    inner_reflector_cell = openmc.Cell(
        name="Inner Reflector", region=inner_reflector_region, fill=m_reflector
    )
...
if (not radial_reflector_is_present) and axial_reflector_is_present:
    ...
    top_reflector_cell = openmc.Cell(
        name="Top Reflector", region=top_reflector_region, fill=m_reflector
    )
    bottom_reflector_cell = openmc.Cell(
        name="Bottom Reflector", region=bottom_reflector_region, fill=m_reflector
    )
    reflector_cells = [top_reflector_cell, bottom_reflector_cell]

if radial_reflector_is_present and axial_reflector_is_present:
    vessel_inner = openmc.ZCylinder(
        x0=vessel_x, y0=vessel_y, r=vessel_inner_radius, boundary_type=vessel_radial_bc
    )  # transmission
    ...
    reflector_cells = [
        outer_reflector_cell,
        inner_reflector_cell,
        top_reflector_cell,
...
for pebble_num, (center, pebble_univ) in enumerate(zip(pebble_centers, pebble_univs)):
    ...
    triso = openmc.model.TRISO(radius_pebble_outer, pebble_univ, center)
...
l_pebble = openmc.model.create_triso_lattice(
    pebble_trisos, llc_vessel, l_pebble_pitch, l_pebble_shape, m_flibe
)
...
source_space = openmc.stats.Box(*vessel_cell.bounding_box, only_fissionable=True)
...
volume_fuel = openmc.VolumeCalculation(
    [m_fuel], 10000000, *c_pebble_central.bounding_box
)
