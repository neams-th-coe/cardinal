...
if len(colors) < len(temporal_plots):
    raise ValueError(
        "Length of 'colors' array must be at least as long as the length "
...
with open(args.logfile_name, "r") as f:
    ...
    for line in lines:
        ...
        if (
            k < len(lines_with_scale_before_write_checkpoint)
            and l == lines_with_scale_before_write_checkpoint[k]
        ):
            k += 1
...
for j in range(n_scalars):
    if "S0" + str(j) in temporal_plots and len(max_scalars[j]) > 1:
        ...
        plt.semilogy(
            ...
            linewidth=lw,
            color=colors[color_idx],
            label="Max " + scalar_names[j],
        )
        color_idx += 1
        print(
            Fore.RED + "\nPercent change in maximum S0" + str(j) + ": ",
            rel_diff_max_S[-1] * 100.0,
        )
        print(
            Fore.GREEN + "  maximum S0" + str(j) + ":                 ",
            max_scalars[j][-1],
        )
