...
program_description = (
    "Script for determining required number of cell divisions " "for an OpenMC model"
)
...
ap.add_argument(
    "-input", dest="input_file", type=str, help="Name of the Cardinal input file to run"
)
...
for i in range(nl):
    ...
    if (not use_saved_statepoints) or (not os.path.exists(output_csv)):
        ...
        os.system(
            ...
            + " -i "
            + input_file
            + " UserObjects/average_power_axial/num_layers="
            + str(n)
            + " Problem/inactive_batches="
            + str(n_inactive)
            + " Problem/batches="
            + str(n_active + n_inactive)
            + " Problem/max_batches="
            + str(n_max_active + n_inactive)
            + " Problem/batch_interval="
            + str(batch_interval)
...
for i in range(1, nl):
    ...
    print("Layers: " + str(n) + ":")
    print(
        "\tPercent change in max power: "
        + str(abs(max_power[i] - max_power[i - 1]) / max_power[i - 1] * 100)
    )
    print(
        "\tPercent change in min power: "
        + str(abs(min_power[i] - min_power[i - 1]) / min_power[i - 1] * 100)
    )
    print(
        "\tPercent change in max power (rel. to avg.): "
        + str(abs(max_power[i] - max_power[i - 1]) / average_power_density * 100)
    )
    print(
        "\tPercent change in min power (rel. to avg.): "
        + str(abs(min_power[i] - min_power[i - 1]) / average_power_density * 100)
    )
...
plt.grid()
plt.savefig(
    "layers_study/" + args.script_name + "_max_power_layers.pdf", bbox_inches="tight"
)
plt.close()
...
plt.grid()
plt.savefig(
    "layers_study/" + args.script_name + "_min_power_layers.pdf", bbox_inches="tight"
)
plt.close()
...
for l in range(nl):
    ...
    plt.plot(
        z, q, label="{} layers".format(i), color=colors[l], marker="o", markersize=2.0
    )
