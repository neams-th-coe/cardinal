...
ap = ArgumentParser()
ap.add_argument(
    "-n", dest="n_axial", type=int, default=40, help="Number of axial cell divisions"
)
ap.add_argument(
    "-s", "--entropy", action="store_true", help="Whether to add a Shannon entropy mesh"
)
args = ap.parse_args()
...
for i in range(N):
    ...
    fuel_cells.append(
        openmc.Cell(fill=uo2, region=-fuel_or & layer, name="Fuel{:n}".format(i))
    )
    gap_cells.append(
        openmc.Cell(
            fill=helium, region=+fuel_or & -clad_ir & layer, name="Gap{:n}".format(i)
        )
    )
    clad_cells.append(
        openmc.Cell(
            fill=zircaloy, region=+clad_ir & -clad_or & layer, name="Clad{:n}".format(i)
        )
    )
    water_cells.append(
        openmc.Cell(
            fill=borated_water,
            region=+clad_or & layer & -box,
            name="Water{:n}".format(i),
        )
    )
