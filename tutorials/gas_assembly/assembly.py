...
def assembly(n_ax_zones, n_inactive, n_active, add_entropy_mesh=False):
    ...
    combined_density = rho * reflector_porosity + matrix_density * (
        1.0 - reflector_porosity
    )
    ...
    c_triso_c_buffer = openmc.Cell(
        name="c_triso_c_buffer", fill=m_graphite_c_buffer, region=+s_fuel & -s_c_buffer
    )
    c_triso_pyc_inner = openmc.Cell(
        name="c_triso_pyc_inner", fill=m_graphite_pyc, region=+s_c_buffer & -s_pyc_inner
    )
    c_triso_sic = openmc.Cell(
        name="c_triso_sic", fill=m_sic, region=+s_pyc_inner & -s_sic
    )
    c_triso_pyc_outer = openmc.Cell(
        name="c_triso_pyc_outer", fill=m_graphite_pyc, region=+s_sic & -s_pyc_outer
    )
    c_triso_matrix = openmc.Cell(
        name="c_triso_matrix", fill=m_graphite_matrix, region=+s_pyc_outer
    )
    ...
    triso_lattice = openmc.model.create_triso_lattice(
        random_trisos, llc, pitch, triso_lattice_shape, m_graphite_matrix
    )
    ...
    for z_min, z_max in zip(axial_coords[0:-1], axial_coords[1:]):
        ...
        coolant_cell.fill = [
            m_coolant for i in range(specs.n_coolant_channels_per_block)
        ]
    ...
    hex_prism = openmc.model.HexagonalPrism(
        bundle_pitch / math.sqrt(3.0), "x", boundary_type="periodic"
    )
    ...
    top_refl_cell = openmc.Cell(
        region=-hex_prism & +max_z & -top_refl, fill=m_reflector
    )
    bottom_refl_cell = openmc.Cell(
        region=-hex_prism & -min_z & +bottom_refl, fill=m_reflector
    )
...
def main():
    ...
    ap.add_argument(
        "-i", dest="n_inactive", type=int, default=500, help="Number of inactive cycles"
    )
    ap.add_argument(
        "-a", dest="n_active", type=int, default=2000, help="Number of active cycles"
    )
