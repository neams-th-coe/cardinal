...
ap = ArgumentParser()
ap.add_argument(
    "-n", dest="n_axial", type=int, default=1, help="Number of axial core divisions"
)
args = ap.parse_args()
...
uo2_u = openmc.Universe(
    cells=[uo2_fuel_cell, gap_1_cell_4, zr_clad_cell_4, h2o_bb_cell_4]
)
...
rod_u = openmc.Universe(
    cells=[rod_fill_cell, tube_clad_cell_1, guide_tube_h2o_bb_cell_1]
)
...
fis_u = openmc.Universe(
    cells=[fission_chamber_cell, tube_clad_cell_2, guide_tube_h2o_bb_cell_2]
)
...
## The assembly.
assembly_bb = openmc.model.RectangularPrism(
    origin=(0.0, 0.0),
    width=16.9 * pitch,
    height=16.9 * pitch,
...
for i in range(core_axial_slices):
    uo2_assembly_cells.append(
        openmc.Cell(
            name="UO2 Assembly Cell " + str(i),
            region=-assembly_bb & +core_z_planes[i] & -core_z_planes[i + 1],
            fill=uo2_assembly,
        )
    )
...
all_cells.append(
    openmc.Cell(
        name="Axial Reflector Cell",
        fill=ref_assembly,
        region=-assembly_bb & -refl_top & +core_z_planes[-1],
    )
)

# --------------------------------------------------------------------------------------------------------------------------#
# Setup the model.
c5g7_model = openmc.Model(
    geometry=openmc.Geometry(openmc.Universe(cells=all_cells)),
    materials=openmc.Materials([uo2, h2o, fiss, zr, al, bc4]),
)

## The simulation settings.
c5g7_model.settings.source = [
    openmc.IndependentSource(
        space=openmc.stats.Box(
            lower_left=(-17.0 * pitch / 2.0, -17.0 * pitch / 2.0, 0.0),
