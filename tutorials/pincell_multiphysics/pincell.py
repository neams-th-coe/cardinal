...
ap = ArgumentParser()
ap.add_argument(
    "-n", dest="n_axial", type=int, default=25, help="Number of axial cell divisions"
)
...
rectangular_prism = openmc.model.RectangularPrism(
    width=pitch, height=pitch, axis="z", origin=(0.0, 0.0), boundary_type="reflective"
)
...
for i in range(N):
    ...
    fuel_cells.append(
        openmc.Cell(fill=uo2, region=-pellet_surface & layer, name="Fuel{:n}".format(i))
    )
    clad_cells.append(
        openmc.Cell(
            fill=zircaloy,
            region=+pellet_surface & -pincell_surface & layer,
            name="Clad{:n}".format(i),
        )
    )
    sodium_cells.append(
        openmc.Cell(
            fill=sodium,
            region=+pincell_surface & layer & -rectangular_prism,
            name="sodium{:n}".format(i),
        )
    )
