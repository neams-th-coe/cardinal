...
MOOSE_DIR = os.getenv(
    "MOOSE_DIR",
    os.path.abspath(os.path.join(os.path.dirname(__name__), "..", "contrib", "moose")),
)
...
if not os.path.exists(large_media) or len(os.listdir(large_media)) == 0:
    raise Exception(
        "To build Cardinal's documentation, MOOSE's large_media submodule must be checked out. Try the following first:\n\ncd cardinal/contrib/moose\ngit submodule update --init large_media"
    )
