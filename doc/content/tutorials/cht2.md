# Tutorial 1B: Conjugate Heat Transfer for Pin Bundle Flow
  id=tutorial1b
...
than model the entire 217-pin bundles, only 7 pins for a single wire pitch (though
the wire is not modeled) are modeled. The spacing between the outermost row of pins and
...
is selected such that the mass flowrate is 0.1 kg/s - this value is not reflective
of the actual [!ac](ABTR) conditions, and is simply set low enough that the flow is
laminar and a turbulence model is not required. Because the multiphysics coupling in
Cardinal is independent of the physics model used in nekRS (aside from the definition
of boundary conditions for exchanging temperature/flux), this simplification does not
detract from the pedagogical purposes of this tutorial.
...
The complete solid mesh is show below; you can generate this mesh by running the
`solid.i` input in `--mesh-only` mode.
The boundary names are illustrated towards
the right by showing only the highlight surface to which each boundary corresponds. Names
...
Again, note that the meshes do not need to share nodes between the fluid and solid
...
system in MOOSE is used to communicatte auxiliary variables across applications; a boundary
...
coupling hierarhcy from a master application to its sub-application. So, the use
...
this limited data transfer option can be used to reduce the mesh interpolation
