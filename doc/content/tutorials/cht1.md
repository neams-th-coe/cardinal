...
!alert note
This particular example uses a large mesh,
...
and opportunities for high-resolution [!ac](T/H) analyses within the MOOSE framework.
...
with this tutorial, as it introduces many concepts taken as understood in
later tutorials.
...
Setting up a coupling of NekRS to MOOSE involves quite minimal user specifications.
...
shown as yellow dashed lines in [top_down]. Due to the top-down nature of [top_down],
flow paths radially (i.e. from lower to higher $r$ at a fixed $z$ between stacks
of reflector blocks) are not shown. While
such radial bypass paths contribute to bypass flow, they are not considered in this example.
...
a characteristic scale. $u_i$ indicates the $i$-th velocity component, $P$ pressure, $\rho$ density, $T$ temperature,
$x_i$ the $i$-th positional coordinate, $L$ a reference length, and $t$ time.
...
among 48 half-reflector blocks) and $A$ is the inlet flow area. Further parametric studies for other
...
where $q^{''}$ is the heat flux, $h$ is the convective heat transfer coefficient,
$T_s$ is the solid temperature, and $T_\infty$
...
often the case for curvilinear applications. [zoom_mesh] shows
a zoom-in of the two mesh files (for the fluid and solid phases). Tather than being limited
...
so we set this element type from Cubit. The `exo2nek` program is part of the tools that
...
function is then used as an initial condition with a
[FunctionIC](https://mooseframework.inl.gov/source/ics/FunctionIC.html).
...
[HeatConduction](https://mooseframework.inl.gov/source/kernels/HeatConduction.html)
 kernel, or $-\nabla\cdot(k\nabla T)=0$. An auxiliary
[DiffusionFluxAux](https://mooseframework.inl.gov/source/auxkernels/DiffusionFluxAux.html) kernel is also specified
...
!alert warning
NekRS is currently limited to a constant time step size. So, while Cardinal technically allows
the time step to be changed in NekRS according to a master application's control,
NekRS's time stepping routines do not properly treat variable time step sizes. This
limitation will be relaxed in the near future.
...
in coupling; that is, the mesh mirror constructed in this case consists of three boundaries of
the original NekRS mesh. The mesh mirror is constructed with
a [NekRSMesh](/mesh/NekRSMesh.md). In order for MOOSE's transfers
...
term that relates the friction pressure drop to a closure, $W$,
...
definition of a friction coefficient, and $V$ is the interstitial velocity [!cite](pronghorn_manual). The present
model can be used to predict $W$ as a function of the block geometry and the Reynolds number.
While we do not perform this data reduction here, we describe the procedure.
...
porous media closures such as effective solid conductivities - that is, parameterize the
