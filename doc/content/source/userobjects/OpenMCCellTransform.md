...
This userobject doesn't translate or rotate the surfaces of the specifeid cell. Instead, if that cell is filled with a universe, the given transform will be used to shift that universe. If you want to use moving mesh capabilities, you may consider using the MOAB skinner with DAGMC geometries instead.
!alert-end!
...
To rotate a universe, the transform array takes ($\phi$, $\theta$, $\psi$) which are the rotation angles in degrees about x, y, and axes. Below is an example which will translate the universe inside a specified OpenMC cell:
