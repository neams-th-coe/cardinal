...
def hydraulic_diameter(
    nrings, bundle_pitch, pin_diameter, wire_diameter=0, wire_pitch=1.0
):
    ...
    wire_length_per_coil = math.sqrt(
        wire_pitch**2 + math.pow(math.pi * (pin_diameter + wire_diameter), 2)
    )
...
for i in range(e_per_bl):
    bl_radii += " " + str(
        math.pow(growth_factor, i) * bl_height + pin_diameter / 2.0 + previous_dx
    )
...
inlet_area = (
    flow_area(n_rings, flat_to_flat, pin_diameter)
    - n_pins * math.pi * (wire_diameter**2) / 4.0
)

if nondimensional:
    hydraulic_d = hydraulic_diameter(
        n_rings, flat_to_flat, pin_diameter, wire_diameter, wire_pitch
    )
else:
    hydraulic_d = 1.0
...
if args.generate:
    ...
    var = os.system(
        home
        + "/cardinal/cardinal-opt -i mesh_info.i fluid.i "
    ...
    var = os.system(
        home
        + "/cardinal/cardinal-opt -i mesh_info.i convert.i "
        + " --mesh-only --n-threads=10"
    )
    if var:
