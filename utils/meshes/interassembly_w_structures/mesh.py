...
bl, bl_outer_dx = bl_points(
    bl_height, growth_factor, e_per_bl, flat_to_flat / 2.0 + thickness
)

# Get the boundary layers on the pad surface
gap_bl, bl_gap_outer_dx = bl_points(
    bl_height,
    growth_factor,
    e_per_gap_bl,
...
bl_pad_ascending, load_pad_bl_dx = bl_points(
    bl_pad_height, growth_factor, e_per_pad_bl, 0.0
)
...
if args.generate:
    ...
    var = os.system(
        home
        + "/cardinal/cardinal-opt -i mesh_info.i bundle.i "
    ...
    print(
        "Inlet flow area:                                ",
        flow_area / (hydraulic_diameter**2),
    )
